...
impl<R: gfx::Resources> gfx_app::Application<R> for App<R> {
    fn new<F: gfx::Factory<R>>(mut factory: F, init: gfx_app::Init<R>) -> Self {
        ...
        let plane = Plane::subdivide(16, 16); // 256 Works good last time, 32 min
...
*/
        ...
        let data = pipe::Data {
            ...
            proj: cgmath::perspective(
                cgmath::deg(60.0f32), init.aspect_ratio, 0.1, 50000.0
                ).into(),
        ...
        let pso = factory.create_pipeline_state(&set, gfx::Primitive::QuadList, gfx::state::Rasterizer::new_fill(),
                               pipe::new()).unwrap();
    ...
    fn render<C: gfx::CommandBuffer<R>>(&mut self, encoder: &mut gfx::Encoder<R, C>) {
        ...
        self.cam_mat.mat  = self.cam_mat.mat *   cgmath::Matrix4::from(cgmath::Matrix3::from_euler(cgmath::Rad{s:(0.0)},cgmath::Rad{s:(self.cam_rot.0*0.0)}, cgmath::Rad{s:(self.cam_rot.0*0.1)}))* cgmath::Matrix4::from(cgmath::Matrix3::from_euler(cgmath::Rad{s:(self.cam_rot.1*-0.1)},cgmath::Rad{s:(self.cam_rot.0*0.0)}, cgmath::Rad{s:(self.cam_rot.0*0.0)}))
                            ;
...
println!("{:?}", self.cam_move);
        ...
        let move_speed = 3.0*1000.0;
        let timedelta = time*0.000001*self.cam_move.length();
